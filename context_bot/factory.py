"""Factory helpers for the context bot."""
...
def _register_routers(dp: Dispatcher) -> None:
    from context_bot.handlers import register_all_routers
...
async def _on_startup(bot: Bot) -> None:
    public_commands = [
        BotCommand(command="start", description="Открыть context bot"),
        BotCommand(command="help", description="Помощь по context bot"),
    ]
    admin_commands = public_commands + [
        BotCommand(command="status", description="Статус bridge-слоя"),
    ]
